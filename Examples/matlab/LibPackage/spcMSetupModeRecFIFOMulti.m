...
function [success, cardInfo] = spcMSetupModeRecFIFOMulti (cardInfo, chEnableH, chEnableL, segmentSize, postSamples, segmentsToRec)
    ...
    % ----- setup the mode -----
    error = error + spcm_dwSetParam_i32 (cardInfo.hDrv, mRegs('SPC_CARDMODE'), mRegs('SPC_REC_FIFO_MULTI'));
    ...
    error = error + spcm_dwSetParam_i64 (cardInfo.hDrv, mRegs('SPC_LOOPS'), segmentsToRec);
