...
# 1 timestamp = 8 bytes (M2i, M3i) or 16 bytes (M4i)
lBytesPerTS = 8
if ((lCardType.value & TYP_SERIESMASK) == TYP_M4IEXPSERIES) or ((lCardType.value & TYP_SERIESMASK) == TYP_M4XEXPSERIES) or ((lCardType.value & TYP_SERIESMASK) == TYP_M2PEXPSERIES):
    lBytesPerTS = 16
    ...
    while qwTotalMem.value < qwToTransfer.value:
        ...
        if dwError != ERR_OK:
            ...
            if lAvailUser.value >= lNotifySize.value:
                ...
                pnData = cast  (addressof (pvBuffer) + lPCPos.value, ptr16) # cast to pointer to 16bit integer
