...
def vDoDataCalculation (lCardType, lMaxDACValue):
    ...
    for i in range (dwSegmentLenSample // 2, dwSegmentLenSample, 1):
        pnData[i] = -lMaxDACValue.value

    vWriteSegmentData (hCard, SEGMENT_IDX.SEG_SYNC, dwSegmentLenSample, pvBuffer)


    # --- ramp up
    dwSegmentLenSample = dwFactor * 64
    for i in range (0, dwSegmentLenSample, 1):
        pnData[i] = int16 (i * dwFShalf.value // dwSegmentLenSample)

    vWriteSegmentData (hCard, SEGMENT_IDX.SEG_RAMPUP, dwSegmentLenSample, pvBuffer)


    # --- ramp down
    dwSegmentLenSample = dwFactor * 64
    for i in range (0, dwSegmentLenSample, 1):
        pnData[i] = int16 (dwFS.value - (i * dwFShalf.value // dwSegmentLenSample))

    vWriteSegmentData (hCard, SEGMENT_IDX.SEG_RAMPDOWN, dwSegmentLenSample, pvBuffer)
    ...
    for i in range (0, dwSegmentLenSample, 1):
        pnData[i] = int16 (dwFShalf.value + int(dwFShalf.value * math.sin (2.0 * math.pi * (i + 0*dwSegmentLenSample) / (dwSegmentLenSample * 4)) + 0.5))
    vWriteSegmentData (hCard, SEGMENT_IDX.SEG_Q1SIN, dwSegmentLenSample, pvBuffer)

    # --- sine, 2nd quarter
    dwSegmentLenSample = dwFactor * 128
    for i in range (0, dwSegmentLenSample, 1):
        pnData[i] = int16 (dwFShalf.value + int(dwFShalf.value * math.sin (2.0 * math.pi * (i + 1*dwSegmentLenSample) / (dwSegmentLenSample * 4)) + 0.5))
    vWriteSegmentData (hCard, SEGMENT_IDX.SEG_Q2SIN, dwSegmentLenSample, pvBuffer)

    # --- sine, 3rd quarter
    dwSegmentLenSample = dwFactor * 128
    for i in range (0, dwSegmentLenSample, 1):
        pnData[i] = int16 (dwFShalf.value + int(dwFShalf.value * math.sin (2.0 * math.pi * (i + 2*dwSegmentLenSample) / (dwSegmentLenSample * 4)) + 0.5))
    vWriteSegmentData (hCard, SEGMENT_IDX.SEG_Q3SIN, dwSegmentLenSample, pvBuffer)

    # --- sine, 4th quarter
    dwSegmentLenSample = dwFactor * 128
    for i in range (0, dwSegmentLenSample, 1):
        pnData[i] = int16 (dwFShalf.value + int(dwFShalf.value * math.sin (2.0 * math.pi * (i + 3*dwSegmentLenSample) / (dwSegmentLenSample * 4)) + 0.5))
    vWriteSegmentData (hCard, SEGMENT_IDX.SEG_Q4SIN, dwSegmentLenSample, pvBuffer)


    # --- DC level
    dwSegmentLenSample = dwFactor * 128
    for i in range (0, dwSegmentLenSample, 1):
        pnData[i] = int16 (dwFS.value // 2)

    vWriteSegmentData (hCard, SEGMENT_IDX.SEG_STOP, dwSegmentLenSample, pvBuffer)
